...
;(function($) {
    ...
    $.smartPop = {
        ...
        open : function(options) {
            ...
            $('#rightScrollBar').css('margin-right', '17px');
            $('#smartPop').show();
            ...
            if(this.opts.bodyClose) {
                ...
                $('body').bind('click', function(event) {
                	...
                	if(target.id == 'smartPop') { 
                    	$.smartPop.close(); 
                    	//smartPop 띄우고 또 띄우는 경우
                    	if($.smartPop.opts.category == 'bigImage'){
                    		introduceTemplate.show();
                            $('#secondStep').show();
                            $('#thirdStep').show();
                    	}
                    }
        ...
        close : function() {
            ...
            if(this.opts.category == 'sns'){
            	if($('#updateBool').val() == 'false'){
            		$('#updateBool').val('true');
            	}
        	}else if(this.opts.category == 'background'){
            	var inner = $('#smartPop_content');
            	$('#contentBackgroundImageWrap').html(inner.html());
            }else if(this.opts.category == 'logo'){
            	var inner = $('#smartPop_content');
            	$('#logoImageWrap').html(inner.html());
            }else if(this.opts.category == 'template'){
            	var inner = $('#smartPop_content');
            	$('#showSmartPop').html(inner.html());
            	$('#secondStep').hide();
                $('#thirdStep').hide();
            }
        },
