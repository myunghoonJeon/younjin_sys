			...
			<ul>	
				<form:form commandName="outboundFilter" method="get">
					<sec:authorize access="hasRole('ROLE_LEVEL4')">
						...
						<br/>
						<li>
							CONT NO : <input type="text" id="contNo" />
						</li>
						<li>
							SEAL NO : <input type="text" id="sealNo"/>
						</li>
						<li>
							CARRIER BOOKING NO : <input type="text" id="carrierBookingNo"/>
						</li>
						<br/>
						<li>
							<span class="house_gbl_addButton yj_button" >add</span>
						</li>
			...
			<table class="yj_table">
				...
				<tbody>
					...
					<c:forEach var="gbl" items="${gblList }">
						...
						<tr class="house_gbl_list_tr" data-seq="${gbl.seq }" data-ws="1">
							<c:if test="${outboundFilter.code eq '' or outboundFilter.code eq null }">
								<td>${gbl.code }</td>
							</c:if>
							<td>
							...
							<td class="house_gbl_no">${(fn:contains(gbl.no, "-house")) ? fn:substring(gbl.no, 0, fn:length(gbl.no) - 7) : gbl.no }</td>
							...
							<td>${gbl.destPort }</td>
							<td class="house_gbl_info_td">
								<input type="text" placeholder="vessel" id="vessel" />
								<input type="text" placeholder="voyage" id="voyage" />
							...
							<c:if test="${fn:contains(gbl.no, '-house1') }">
								<td class="house_seperate_merge"><img src="${cp }/resources/images/merge.gif" /></td>
							</c:if>
						</tr>
					</c:forEach>
				</tbody>
